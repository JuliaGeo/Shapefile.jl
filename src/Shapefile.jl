...
module Shapefile
    ...
    function Base.read(io::IO,::Type{Polyline{T}}) where T
        ...
        parts = Array{Int32}(undef, numparts)
        read!(io, parts)
        points = Array{Point{T}}(undef, numpoints)
        read!(io, points)
    ...
    function Base.read(io::IO,::Type{PolylineM{T,M}}) where {T,M}
        ...
        points = Array{Point{T}}(undef, numpoints)
        read!(io, points)
        mrange = Array{M}(undef, 2)
        read!(io, mrange)
        measures = Array{M}(undef, numpoints)
        read!(io, measures)
    ...
    function Base.read(io::IO,::Type{PolylineZ{T,M}}) where {T,M}
        ...
        points = Array{Point{T}}(undef, numpoints)
        read!(io, points)
        zrange = Array{T}(undef, 2)
        read!(io, zrange)
        zvalues = Array{T}(undef, numpoints)
        read!(io, zvalues)
        mrange = Array{M}(undef, 2)
        read!(io, mrange)
        measures = Array{M}(undef, numpoints)
    ...
    function Base.read(io::IO,::Type{MultiPatch{T,M}}) where {T,M}
        ...
        parts = Array{Int32}(undef, numparts)
        read!(io, parts)
        parttypes = Array{Int32}(undef, numparts)
        read!(io, parttypes)
        points = Array{Point{T}}(undef, numpoints)
        read!(io, points)
        zrange = Array{T}(undef, 2)
